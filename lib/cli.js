...
var fs = require('fs'),
    ...
    argv = require('yargs').usage('Usage: $0 <command> [options]').command('enable', 'Enable autostart here with a custom key', function (yargs) {
  argv = yargs.option('n', {
    demand: true,
    alias: 'name',
    ...
    type: 'string',
    default: process.cwd()
  }).help('h').alias('h', 'help').showHelpOnFail(false, 'Use --help for further information').argv;
...
}).demand(1).help('h').alias('h', 'help').showHelpOnFail(false, 'Use --help for further information').argv;