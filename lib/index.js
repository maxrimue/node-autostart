...
function enableAutostart(key, command, path, callback) {
  ...
  if (!key || !command || !path || !callback) {
    console.error('Not enough arguments passed to enableAutostart().'); return 1;
  }

  if (typeof(key) !== 'string') {
    console.error('Passed "key" to enableAutostart() is not a string.'); return 1;
  }

  if (typeof(command) !== 'string') {
    console.error('Passed "command" to enableAutostart() is not a string.'); return 1;
  }

  if (typeof(path) !== 'string') {
    console.error('Passed "path" to enableAutostart() is not a string.'); return 1;
  }

  if (typeof(callback) !== 'function') {
    console.error('Passed "callback" to enableAutostart() is not a function.'); return 1;
  }

  isAutostartEnabled(key, function(isEnabled, error) {
    if (error) callback(err);

    if (isEnabled) callback('Autostart already is enabled');
...
function disableAutostart(key, callback) {
  ...
  if (!key || !callback) {
    console.error('Not enough arguments passed to disableAutostart()'); return 1;
  }

  if (typeof(key) !== 'string') {
    console.error('Passed "key" to disableAutostart() is not a string.'); return 1;
  }

  if (typeof(callback) !== 'function') {
    console.error('Passed "callback" to disableAutostart() is not a function.'); return 1;
  }

  isAutostartEnabled(key, function(isEnabled, error) {
    if (error) callback(error);

    if (!isEnabled) callback('Autostart is not enabled, so you cannot disable it');
...
function isAutostartEnabled(key, callback) {
  ...
  if (!key || !callback) {
    console.error('Not enough arguments passed to isAutostartEnabled()'); return 1;
  }
  ...
  if (os.platform() === "darwin") {
    ...
    callback(false, 'Your platform currently is not supported');
  }
}
